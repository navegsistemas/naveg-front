...
- **Quando o domínio da API de homologação vier** (ex.: `api-homolog.<domínio>`), fixo na `main` da API: a `PUBLIC_URL_DA_API` do *Preview* do front passa para ele; a origem do front de homologação sai
...
- **Rotação**: chave JSON a cada 90 dias (as duas refeitas em 2026-09-25: vencem por volta de 2026-12-24); tokens do Upstash e o
...
| **0 · a casa** | 2FA; rulesets na `main`; ~~CI de PR nos dois repositórios (verificar, build, auditoria, **fumaça no preview**); Dependabot~~ *(2026-09-25: naveg-api-vercel#6 e naveg-front#7; a fumaça roda em todo deploy da API, e já pegou uma chave morta no Preview; falta marcá-los como obrigatórios nos rulesets, e o token do Dependabot para o npm da API)*; push protection; `CODEOWNERS`; a API inteira subindo contra o emulador | nada — é gratuito e não muda o que está no ar |
| **0b · o contrato no CI** *(novo, era da fase 3)* | ~~`fluviapp-kmp` e `fluviapp` em `navegsistemas`~~ (feito em 2026-09-23); ~~token da org só de leitura; os jobs **contrato** e **emulador** no CI daqui e da API~~ — ✅ **feita em 2026-09-25** (naveg-front#17, naveg-api-vercel#13): 44 cenários do contrato e o de ponta a ponta no emulador rodando em todo PR, nenhum pulado. O `FLUVIAPP_LEITURA_TOKEN` (segredo da org, em Actions e Dependabot) vence em 90 dias | nada |
| **1 · homologação estável** | branch `producao` criada a partir da `main` e configurada como *Production Branch* — *(2026-09-24: criada nos dois repositórios, com o ruleset; Production Branch apontada para ela na Vercel, na API e no front (`naveg-front-agencia`, com `naveg-front-agencia.vercel.app` fixo na `main`); o domínio oficial entra no lançamento de produção)*; domínios fixos de homologação *(o do front é o `naveg-front-agencia.vercel.app`; o da API fica para depois — 2026-09-25: a homologação provisória do §3 vale até o lançamento)*; ~~widget de homologação do Turnstile; Upstash de homologação; `ORIGENS_PERMITIDAS` e `PUBLIC_URL_DA_API` por ambiente~~ (2026-09-25); a trava ambiente × projeto (e chave Web), com a exceção do provisório | D3; D4 só para o domínio da API |
...
| **D4** | Os domínios | *(2026-09-24)* Em parte: produção do front em `gruponaveg.com.br` (DNS no lançamento); homologação do front em `naveg-front-agencia.vercel.app`; o domínio da API de homologação **fica para depois** *(2026-09-25)*: a homologação provisória (§3) é o desenho até o lançamento, e os domínios entram juntos, um por ambiente. A API de produção ganha domínio no lançamento. Recomendação original: Recomendação: `agencia.naveg.com.br`/`api.naveg.com.br` em produção; `homolog.` e `api-homolog.` em homologação |
