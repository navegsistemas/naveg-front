...
## Passo 11 — Handoff para o WhatsApp · ✅ código · ⏳ o número do atendimento
...
> - **O número do atendimento ainda não chegou.** `WHATSAPP_DAS_RESERVAS` (em `conteudo/atendimento.ts`) é o do
>   atendente, que está `null`: preenchê-lo liga o botão no totem e na seção de atendimento de uma vez.
